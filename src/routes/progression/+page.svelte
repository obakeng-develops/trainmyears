<script lang="ts">
	...
	let mode = $state<'listen' | 'trainer'>('listen');
	let tonic = $state('0');
	let bpm = $state(96);
	...
	const triggerPreload = () => {
		...
		void (async () => {
			try { await (engine as any).loadSampler?.(); } catch { /* ignore */ }
		})();
	};
	...
	const settingsKey = 'progression-quickstart';
	...
	$effect(() => {
		...
		window.localStorage.setItem(settingsKey, JSON.stringify({ mode, tonic, bpm, presetId, droneOn }));
	});
...
<div class="min-h-screen px-6 py-10 pb-32 lg:px-10 lg:pb-10" role="group" onpointerenter={triggerPreload} onpointerdown={triggerPreload}>
	<div class="mx-auto flex max-w-3xl flex-col gap-8">
		<header class="space-y-3">
			<div class="text-xs font-semibold uppercase tracking-[0.3em] text-muted-foreground">
				Progression · Quickstart
			</div>
			...
			<p class="max-w-xl text-sm text-muted-foreground md:text-base">
				Loop chord progressions against a tonic drone, then test your ear.
			</p>
		</header>
		...
		<Card.Root class="border/60 bg-card/80 shadow-none backdrop-blur lg:shadow-xl">
			<Card.Header class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
				<div>
					<Card.Title class="font-display text-xl">Progression loop</Card.Title>
					<Card.Description>
						{mode === 'trainer'
							? 'Identify the function of the highlighted chord.'
							: 'Notice the pull and release against the drone.'}
					</Card.Description>
				</div>
			...
			<Card.Content class="space-y-6">
				...
				>
					{#each progressionChords as chord, index (`${presetId}-${index}`)}
						...
						>
							{#if mode === 'trainer'}
								<div class="text-sm font-semibold">{isTarget ? '?' : index + 1}</div>
							{:else}
								<div class="text-sm font-semibold">{functionLabel(chord.rootPc)}</div>
							{/if}
						</div>
					{/each}
				</div>
						...
						{#if trainerTarget < 0}
							<div class="text-center">
								<div class="text-sm text-muted-foreground">
									Hear a progression, then name the highlighted chord's function.
								</div>
								<Button class="mt-4" onclick={() => void startTrainer()}>Start</Button>
							</div>
						{:else}
							<div class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
								What function is the highlighted chord?
							</div>
				...
				<div class="space-y-2">
					...
					<ToggleGroup.Root type="single" bind:value={presetId} class="flex flex-wrap gap-2">
						{#each presetProgressions as p}
							<ToggleGroup.Item value={p.id} class="px-3 text-xs">{p.label}</ToggleGroup.Item>
						{/each}
					</ToggleGroup.Root>
				</div>
				...
				<div class="grid gap-4 md:grid-cols-3">
					<div class="rounded-xl border border-border/60 bg-[var(--surface-2)] px-4 py-3">
						<div class="text-xs font-semibold tracking-wide text-muted-foreground uppercase">Key</div>
					...
					<div class="rounded-xl border border-border/60 bg-[var(--surface-2)] px-4 py-3">
						<div class="flex items-center justify-between text-xs text-muted-foreground">
							<span class="font-semibold uppercase tracking-wide">Tempo</span>
							<span class="text-foreground">{bpm}</span>
						</div>
						<Slider type="single" class="mt-2" min={60} max={140} step={1} bind:value={bpm} />
					</div>
					<div class="flex items-center justify-between rounded-xl border border-border/60 bg-[var(--surface-2)] px-4 py-3">
						<span class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Drone</span>
